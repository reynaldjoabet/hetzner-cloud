...
object HetznerCloud extends App {
  ...
  val certificate = Certificate(
    ...
    fingerprint = "AB:CD:EF:12:34:56:78:90:AB:CD:EF:12:34:56:78:90:AB:CD:EF:12:34:56:78:90",
    id = 123456,
  ...
  val pricePerTime =
    ListLoadBalancerTypes200ResponseLoadBalancerTypesInnerPricesInner(
      ...
      location = "fsn1",
      priceHourly = ListLoadBalancerTypes200ResponseLoadBalancerTypesInnerPricesInnerPriceHourly(
        net = BigDecimal(0.012),
        gross = BigDecimal(0.01428)
      ),
      priceMonthly = ListLoadBalancerTypes200ResponseLoadBalancerTypesInnerPricesInnerPriceMonthly(
        net = BigDecimal(8.0),
        gross = BigDecimal(9.52)
      ),
      pricePerTbTraffic = ListLoadBalancerTypes200ResponseLoadBalancerTypesInnerPricesInnerPricePerTbTraffic(
        net = BigDecimal(1.0),
  ...
  val zone = PrimaryZone(
    ...
    name = "example.com",
    protection = ListFloatingIps200ResponseFloatingIpsInnerProtection(delete = false),
    recordCount = 42,
...
//All our servers will be connected to this private network, and external access to internal resources will only be possible through VPN or Load Balancers.
  ...
  val createVpnServerRequest = CreateServerRequest(
    ...
    location = Some("fsn1"),
    networks = Some(List( /* ID of the private network created earlier */ 123456)),
    placementGroup = None,
    publicNet = Some(
      CreateServerRequestPublicNet(
...
//   )
  ...
  val mmanagementlbrequest = CreateLoadBalancerRequest(
    ...
    labels = Some(Map("environment" -> "production", "role" -> "management-lb")),
    location = Some("fsn1"),
    network = Some( /* ID of the private network created earlier */ 123456),
    networkZone = None,
    publicInterface = Some(false),
  ...
  val managementlbTarget = LoadBalancerTarget1(
    ...
    labelSelector = None,
    server = Some(
      LoadBalancerTargetServer1( /* ID of the management server created earlier */ 123456)
    ),
    usePrivateIp = Some(true)
  )
    ...
    rules = Some(
      List(
        ...
        Rule(
          ...
          protocol = RuleEnums.Protocol.tcp,
          port = Some("22"),
          sourceIps = Some(List("203.0.113.1/32")), // Use your ACTUAL office/home IP
...
// 		Service C (API): Listens on port 8443 for a specific mobile API.
  ...
  val kubelbTarget = LoadBalancerTarget1(
    ...
    labelSelector = None,
    server = Some(
      LoadBalancerTargetServer1( /* ID of one of your Kubernetes servers */ 123456)
    ),
    usePrivateIp = Some(true)
  )
...
//firewall_attachment
  val bulkNetworkAttachments = (1 to 10).map(x =>
...
//firewall rule to allow requests only from cloudflare ips
  ...
  val masterLoadBalancerTarget = LoadBalancerTarget1(
    ...
    server = Some(
      LoadBalancerTargetServer1( /* ID of one of your Kubernetes master servers */ 123456)
    ),
    usePrivateIp = Some(true)
  )
    ...
    rules = Some(
      List(
        ...
        Rule(
          ...
          description = Some("Allow Kubernetes API access from management IPs only")
        ),
        Rule(
          direction = RuleEnums.Direction.in,
          protocol = RuleEnums.Protocol.tcp,
...
//Set the Cluster CIDR to 10.244.0.0/16
  ...
  val mySubnet = NetworkCreateRequestSubnetsInner(
    networkZone = "eu-central",
    `type` = NetworkCreateRequestSubnetsInnerEnums.Type.vswitch, // Link to Dedicated world
    ipRange = Some("10.0.1.0/24"),
  ...
  val firewallRules = List(
    ...
    Rule(
      description = Some("Allow all TCP traffic between nodes on the private network"),
      direction = RuleEnums.Direction.in,
      protocol = RuleEnums.Protocol.tcp,
      port = None,
      sourceIps = Some(List("10.10.10.0/24"))
    ...
    Rule(
      description = Some("Allow all UDP traffic between nodes on the private network"),
      direction = RuleEnums.Direction.in,
      protocol = RuleEnums.Protocol.udp,
      port = None,
    ...
    Rule(
      description = Some("Allow port 6443 (Kubernetes API server) between masters"),
      direction = RuleEnums.Direction.in,
      protocol = RuleEnums.Protocol.tcp,
      port = Some("6443"),
      sourceIps = Some(List("0.0.0.0/0", "::/0"))
    ...
    Rule(
      description = Some("Allow traffic between nodes for peer-to-peer image distribution"),
      direction = RuleEnums.Direction.in,
      protocol = RuleEnums.Protocol.tcp,
      port = Some("5001"),
      sourceIps = Some(List("0.0.0.0/0", "::/0"))
  ...
  val exampleRrsetA = CreateZoneRequestRrsetsInner(
    name = "www",
    `type` = CreateZoneRequestRrsetsInnerEnums.Type.A,
    records = Seq(
      Record(value = "203.0.113.1", comment = Some("primary web server")),
      Record(value = "203.0.113.2", comment = None)
    ),
    ttl = Some(3600),
    labels = None
  )

  val exampleApexRrset = CreateZoneRequestRrsetsInner(
    name = "@",
    `type` = CreateZoneRequestRrsetsInnerEnums.Type.A,
    records = Seq(Record(value = "203.0.113.1", comment = None)),
    ttl = None,
    labels = None
  )

  val createPrimaryZoneRequest = CreateZoneRequest(
    name = "example.com",
    mode = CreateZoneRequestEnums.Mode.primary,
    ttl = Some(3600),
    labels = Some(Map("environment" -> "production")),
    primaryNameservers = None,
    rrsets = Some(Seq(exampleApexRrset, exampleRrsetA)),
    zonefile = None
  )

  val secondaryZonePrimaryNameserver = PrimaryZoneAllOfPrimaryNameserversInner(
  ...
  val createSecondaryZoneRequest = CreateZoneRequest(
    name = "secondary-example.com",
    mode = CreateZoneRequestEnums.Mode.secondary,
    ttl = None,
    labels = Some(Map("environment" -> "production")),
    primaryNameservers = Some(Seq(secondaryZonePrimaryNameserver)),
    rrsets = None,
    zonefile = None
  )

  val changeZonePrimaryNameserversRequest = ChangeZonePrimaryNameserversRequest(
    primaryNameservers = Seq(secondaryZonePrimaryNameserver)
  )

  val changeZoneTtlRequest = ChangeZoneTtlRequest(ttl = 7200)

  val changeZoneRrsetTtlRequest = ChangeZoneRrsetTtlRequest(ttl = 1800)

  // Update requests -------------------------------------------------------
  // Update*Request bodies only ever set `name`/`labels` (or similarly narrow
  // fields): fields left as None are left unchanged server-side, but any
  // `labels` map you do send replaces the existing one entirely rather than
  // merging into it.

  val updateServerRequest = UpdateServerRequest(
    name = Some("renamed-server"),
    labels = Some(Map("environment" -> "production", "role" -> "web"))
  )

  val updateSshKeyRequest = UpdateSshKeyRequest(
    name = Some("renamed-ssh-key"),
    labels = None
  )

  val updateVolumeRequest = UpdateVolumeRequest(
    name = Some("renamed-volume"),
    labels = None
  )

  val updateFirewallRequest = UpdateFirewallRequest(
    name = Some("renamed-firewall"),
    labels = Some(Map("environment" -> "production"))
  )

  val updatePlacementGroupRequest = UpdatePlacementGroupRequest(
    name = Some("renamed-placement-group"),
    labels = None
  )

  val updateCertificateRequest = UpdateCertificateRequest(
    name = Some("renamed-certificate"),
    labels = None
  )

  val updateImageRequest = UpdateImageRequest(
    description = Some("Golden image for web servers"),
    `type` = Some(UpdateImageRequestEnums.Type.snapshot),
    labels = Some(Map("environment" -> "production"))
  )

  // Change requests ---------------------------------------------------------

  val changeServerTypeRequest = ChangeServerTypeRequest(
    upgradeDisk = false, // keep upgradeDisk = false to allow downgrading later
    serverType = "cx33"
  )
  ...
  val changeServerAliasIpsRequest = ChangeServerAliasIpsRequest(
    network = 123456,
    aliasIps = Seq("10.0.1.20", "10.0.1.21")
  )

  val changeNetworkIpRangeRequest = ChangeIPRangeRequest(
    ipRange = "10.0.0.0/16"
  )
