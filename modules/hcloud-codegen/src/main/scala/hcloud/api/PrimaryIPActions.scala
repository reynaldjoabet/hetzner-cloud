...
import hcloud.models.ListFloatingIps200ResponseFloatingIpsInnerProtection
import hcloud.models.PrimaryIPActionsAssignRequest
...
object PrimaryIPActions:
  def apply(
      baseUrl: String = "https://api.hetzner.cloud/v1"
  ): PrimaryIPActions[hcloud.Authorization.NoAuthorization.type] =
    PrimaryIPActions(baseUrl, hcloud.Authorization.NoAuthorization)
  def withBasicAuth(
      baseUrl: String,
      username: String,
      password: String
  ): PrimaryIPActions[hcloud.Authorization.BasicAuth] =
    PrimaryIPActions(
      baseUrl,
  ...
  def withApiKeyAuth(
      baseUrl: String,
      apiKey: String
  ): PrimaryIPActions[hcloud.Authorization.ApiKey] =
    PrimaryIPActions(baseUrl, hcloud.Authorization.ApiKey(apiKey))

  def withBearerTokenAuth(
      baseUrl: String,
      token: String
  ): PrimaryIPActions[hcloud.Authorization.BearerToken] =
    PrimaryIPActions(baseUrl, hcloud.Authorization.BearerToken(token))

case class PrimaryIPActions[Auth <: hcloud.Authorization] private (
    baseUrl: String,
  ...
  def withNoAuth: PrimaryIPActions[hcloud.Authorization.NoAuthorization.type] =
  ...
  /** Assign a [Primary IP](#tag/primary-ips) to a resource. A
    * [Server](#tag/servers) can only have one [Primary IP](#tag/primary-ips) of
    * type `ipv4` and one of type `ipv6` assigned. If you need more IPs use
    * [Floating IPs](#tag/floating-ips). A [Server](#tag/servers) must be
    * powered off (status `off`) in order for this operation to succeed. ####
    * Operation specific errors | Status | Code | Description | | --- | --- |
    * --- | | `422` | `server_not_stopped` | The [Server](#tag/servers) is
    * running, but needs to be powered off | | `422` |
    * `primary_ip_already_assigned` | [Primary IP](#tag/primary-ips) is already
    * assigned to a different [Server](#tag/servers) | | `422` |
    * `server_has_ipv4` | The [Server](#tag/servers) already has an IPv4 address | |
    * `422` | `server_has_ipv6` | The [Server](#tag/servers) already has an IPv6
    * address |
    *
  ...
  /** Change the reverse DNS records for this [Primary IP](#tag/primary-ips).
    ...
    *   For IPv4 addresses this must be the exact address of the [Primary
    *   IP](#tag/primary-ips). For IPv6 addresses this must be a single address
    *   within the `/64` subnet of the [Primary IP](#tag/primary-ips). The
    *   `dns_ptr` attribute specifies the hostname used for the IP address. Must
    *   be a fully qualified domain name (FQDN) without trailing dot. For IPv6
    *   [Primary IPs](#tag/primary-ips) up to 100 entries can be created.
    */
  def changePrimaryIpDnsPtr(
      id: Long,
      listFloatingIps200ResponseFloatingIpsInnerDnsPtrInner: ListFloatingIps200ResponseFloatingIpsInnerDnsPtrInner
  )(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[Either[ResponseException[String], ActionResponse1]] =
    val idPathParam =
  ...
  /** Changes the protection configuration of a [Primary IP](#tag/primary-ips).
    * A [Primary IPs](#tag/primary-ips) deletion protection can only be enabled
    * if its `auto_delete` property is set to `false`.
    *
    * Expected answers: code 201 : ActionResponse1 (Request succeeded.) code 4xx
    * : GetActions4xxResponse (Request failed with a user error.) code 5xx :
    * GetActions5xxResponse (Request failed with a server error.)
  ...
  /** Returns a specific [Action](#tag/actions) for a [Primary
    * IP](#tag/primary-ips).
    *
    ...
    *   ID of the Action.
    */
  def getPrimaryIpAction(id: Long, actionId: Long)(using
      Auth <:< hcloud.Authorization.BearerToken
    ...
    val requestURL =
      uri"$baseUrl/primary_ips/${idPathParam}/actions/${actionIdPathParam}"
  ...
  /** Returns a single [Action](#tag/actions).
    ...
    *   ID of the Action.
    */
  def getPrimaryIpsAction(id: Long)(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[Either[ResponseException[String], ActionResponse]] =
    val idPathParam =
      PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
  ...
  def listPrimaryIpActions(
    ...
    val idPathParam =
      PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
    val requestURL =
      uri"$baseUrl/primary_ips/${idPathParam}/actions"
        .addParams(
          FormSerializable
  ...
  /** Unassign a [Primary IP](#tag/primary-ips) from a resource. A
    * [Server](#tag/servers) must be powered off (status `off`) in order for
    * this operation to succeed. A [Server](#tag/servers) requires at least one
    * network interface (public or private) to be powered on. #### Operation
    * specific errors | Status | Code | Description | | --- | --- | --- | |
    * `422` | `server_not_stopped` | The [Server](#tag/servers) is running, but
    * needs to be powered off | | `422` | `server_is_load_balancer_target` | The
    * [Server](#tag/servers) IPv4 address is a loadbalancer target |
    *
    * Expected answers: code 201 : ActionResponse1 (Request succeeded.) code 4xx
    * : GetActions4xxResponse (Request failed with a user error.) code 5xx :
    * GetActions5xxResponse (Request failed with a server error.)
    ...
    *   ID of the Primary IP.
    */
  def unassignPrimaryIp(id: Long)(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[Either[ResponseException[String], ActionResponse1]] =
    val idPathParam =
      PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
