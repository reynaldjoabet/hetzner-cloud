...
import hcloud.models.PrimaryIPCreateRequest
import hcloud.models.PrimaryIPResponse
import hcloud.models.PrimaryIPUpdateRequest
import hcloud.models.PrimaryIPsResponse
...
object PrimaryIPs:
  def apply(baseUrl: String = "https://api.hetzner.cloud/v1"): PrimaryIPs[hcloud.Authorization.NoAuthorization.type] = PrimaryIPs(baseUrl, hcloud.Authorization.NoAuthorization)
  def withBasicAuth(baseUrl: String, username: String, password: String): PrimaryIPs[hcloud.Authorization.BasicAuth] =
    PrimaryIPs(baseUrl, hcloud.Authorization.BasicAuth(username, password))

  def withApiKeyAuth(baseUrl: String, apiKey: String): PrimaryIPs[hcloud.Authorization.ApiKey] =
    PrimaryIPs(baseUrl, hcloud.Authorization.ApiKey(apiKey))

  def withBearerTokenAuth(baseUrl: String, token: String): PrimaryIPs[hcloud.Authorization.BearerToken] =
    PrimaryIPs(baseUrl, hcloud.Authorization.BearerToken(token))

case class PrimaryIPs[Auth <: hcloud.Authorization] private (baseUrl: String, authConfig: hcloud.Authorization):
  def withBasicAuth(username: String, password: String): PrimaryIPs[hcloud.Authorization.BasicAuth] =
  ...
  def withBearerTokenAuth(token: String): PrimaryIPs[hcloud.Authorization.BearerToken] =
   ...
   * 
   * Expected answers:
   *   code 201 : CreatePrimaryIPResponse (Request succeeded.)
   ...
   *   APIToken (http)
   * 
   * @param primaryIPCreateRequest Request Body for creating a new [Primary IP](#tag/primary-ips).  The `location`, `datacenter` and `assignee_id`/`assignee_type` attributes are mutually exclusive. 
   */
  def createPrimaryIp(primaryIPCreateRequest: PrimaryIPCreateRequest)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], CreatePrimaryIPResponse]] =
    val requestURL =
    ...
    basicRequest
      ...
      .auth(authConfig)
      .body(asJson(primaryIPCreateRequest))
   ...
   */
  def deletePrimaryIp(id: Long)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], Unit]] =
   ...
   * 
   * Expected answers:
   *   code 200 : PrimaryIPResponse (Request succeeded.)
   *   code 4xx : GetActions4xxResponse (Request failed with a user error.)
   *   code 5xx : GetActions5xxResponse (Request failed with a server error.)
   * 
   * Available security schemes:
   ...
   * @param id ID of the Primary IP.
   */
  def getPrimaryIp(id: Long)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], PrimaryIPResponse]] =
    val idPathParam = PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
    val requestURL =
      uri"$baseUrl/primary_ips/${idPathParam}"
   ...
   */
  def listPrimaryIps(name: Option[String] = scala.None, labelSelector: Option[String] = scala.None, ip: Option[String] = scala.None, page: Option[Long] = scala.None, perPage: Option[Long] = scala.None, sort: Seq[String])(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], PrimaryIPsResponse]] =
    val requestURL =
      uri"$baseUrl/primary_ips"
    ...
    basicRequest
      ...
      .contentType("application/json")
      .auth(authConfig)
      .response(asJson[PrimaryIPsResponse])
   ...
   * 
   * @param id ID of the Primary IP.
   * @param primaryIPUpdateRequest 
   */
  def updatePrimaryIp(id: Long, primaryIPUpdateRequest: PrimaryIPUpdateRequest)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], PrimaryIPResponse]] =
    val idPathParam = PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
    val requestURL =
      uri"$baseUrl/primary_ips/${idPathParam}"
    ...
    basicRequest
      ...
      .contentType("application/json")
      .auth(authConfig)
      .body(asJson(primaryIPUpdateRequest))
      .response(asJson[PrimaryIPResponse])
