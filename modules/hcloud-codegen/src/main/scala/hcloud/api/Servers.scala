...
object Servers:
  ...
  ): Servers[hcloud.Authorization.NoAuthorization.type] =
    Servers(baseUrl, hcloud.Authorization.NoAuthorization)
  def withBasicAuth(
      baseUrl: String,
      username: String,
      password: String
  ): Servers[hcloud.Authorization.BasicAuth] =
    Servers(baseUrl, hcloud.Authorization.BasicAuth(username, password))

  def withApiKeyAuth(
      baseUrl: String,
      apiKey: String
  ): Servers[hcloud.Authorization.ApiKey] =
    Servers(baseUrl, hcloud.Authorization.ApiKey(apiKey))

  def withBearerTokenAuth(
      baseUrl: String,
      token: String
  ): Servers[hcloud.Authorization.BearerToken] =
    Servers(baseUrl, hcloud.Authorization.BearerToken(token))

case class Servers[Auth <: hcloud.Authorization] private (
    baseUrl: String,
  ...
  /** Creates a new Server. Returns preliminary information about the Server as
    * well as an Action that covers progress of creation. #### Operation
    * specific errors | Status | Code | Description | | --- | --- | --- | |
    * `412` | `primary_ip_version_mismatch` | The specified Primary IP has the
    * wrong IP Version | | `422` | `placement_error` | An error during the
    * placement occurred | | `422` | `primary_ip_assigned` | The specified
    * Primary IP is already assigned to a server | | `422` |
    * `primary_ip_datacenter_mismatch` | he specified Primary IP is in a
    * different datacenter |
    *
    * Expected answers: code 201 : CreateServerResponse (Request succeeded.)
    ...
    *   listed in `/locations`. Some properties like `start_after_create` or
    *   `automount` will trigger Actions after the Server is created. Those
    *   Actions are listed in the `next_actions` field in the response. For
    *   accessing your Server we strongly recommend to use SSH keys by passing
    *   the respective key IDs in `ssh_keys`. If you do not specify any
    *   `ssh_keys` we will generate a root password for you and return it in the
    *   response. Please note that provided user-data is stored in our systems.
    *   While we take measures to protect it we highly recommend that you don’t
    *   use it to store passwords or other sensitive information.
    */
  def createServer(createServerRequest: CreateServerRequest)(using
  ...
  /** Deletes a Server. This immediately removes the Server from your account,
    ...
    *
    * Expected answers: code 200 : DeleteServer200Response (Request succeeded.)
    * code 4xx : GetActions4xxResponse (Request failed with a user error.) code
    * 5xx : GetActions5xxResponse (Request failed with a server error.)
    *
    * Available security schemes: APIToken (http)
    ...
    *   ID of the Server.
    */
  def deleteServer(id: Long)(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[
    Either[ResponseException[String], DeleteServer200Response]
  ] =
    val idPathParam =
  ...
  /** Returns a specific Server object. The Server must exist inside the
    * Project.
    *
    * Expected answers: code 200 : GetServer200Response (Request succeeded.)
    * code 4xx : GetActions4xxResponse (Request failed with a user error.) code
    * 5xx : GetActions5xxResponse (Request failed with a server error.)
    *
    * Available security schemes: APIToken (http)
    ...
    *   ID of the Server.
    */
  def getServer(id: Long)(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[
    Either[ResponseException[String], GetServer200Response]
  ] =
    val idPathParam =
      PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
    val requestURL =
  ...
  /** Get Metrics for specified Server. You must specify the type of metric to
    ...
    *
    * Expected answers: code 200 : GetLoadBalancerMetrics200Response (Request
    ...
    * @param start
    *   Start of period to get Metrics for (must be in
    *   [RFC3339](https://datatracker.ietf.org/doc/html/rfc3339#section-5.6)
    *   format).
    * @param end
    *   End of period to get Metrics for (must be in
    *   [RFC3339](https://datatracker.ietf.org/doc/html/rfc3339#section-5.6)
    *   format).
    * @param step
  ...
  def getServerMetrics(
    ...
    Either[ResponseException[String], GetLoadBalancerMetrics200Response]
  ] =
    val idPathParam =
      PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
    val requestURL =
      uri"$baseUrl/servers/${idPathParam}/metrics"
        ...
        .addParams(
          FormSerializable
            .serialize("start", start, FormStyleFormat.FORM, true): _*
        )
        .addParams(
          FormSerializable.serialize("end", end, FormStyleFormat.FORM, true): _*
        )
        .addParams(
          FormSerializable
            .serialize("step", step, FormStyleFormat.FORM, true): _*
        )
  ...
  /** Returns all existing Server objects.
    *
    * Expected answers: code 200 : ListServers200Response (Request succeeded.)
    * Headers : x-next - A link to the next page of responses. code 4xx :
    ...
    *   Selector](#description/label-selector)\".
    * @param sort
    *   Sort resources by field and direction. May be used multiple times. For
    *   more information, see \"[Sorting](#description/sorting)\".
    * @param status
    *   Filter resources by status. May be used multiple times. The response
  ...
  def listServers(
    ...
    Either[ResponseException[String], ListServers200Response]
  ] =
    val requestURL =
      uri"$baseUrl/servers"
        .addParams(
  ...
  /** Updates a Server. You can update a Server’s name and a Server’s labels.
    ...
    */
  def updateServer(id: Long, updateServerRequest: UpdateServerRequest)(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[
    Either[ResponseException[String], GetServer200Response]
  ] =
