...
object FirewallActions:
  ...
  ): FirewallActions[hcloud.Authorization.NoAuthorization.type] =
    FirewallActions(baseUrl, hcloud.Authorization.NoAuthorization)
  def withBasicAuth(
      baseUrl: String,
      username: String,
      password: String
  ): FirewallActions[hcloud.Authorization.BasicAuth] =
    FirewallActions(baseUrl, hcloud.Authorization.BasicAuth(username, password))

  def withApiKeyAuth(
      baseUrl: String,
      apiKey: String
  ): FirewallActions[hcloud.Authorization.ApiKey] =
    FirewallActions(baseUrl, hcloud.Authorization.ApiKey(apiKey))

  def withBearerTokenAuth(
      baseUrl: String,
      token: String
  ): FirewallActions[hcloud.Authorization.BearerToken] =
    FirewallActions(baseUrl, hcloud.Authorization.BearerToken(token))

case class FirewallActions[Auth <: hcloud.Authorization] private (
    baseUrl: String,
  ...
  /** Applies a [Firewall](#tag/firewalls) to multiple resources. Supported
    * resources: - [Servers](#tag/servers) (with a public network interface) -
    * [Label Selectors](#description/label-selector) A [Server](#tag/servers)
    * can be applied to [a maximum of 5
    * Firewalls](https://docs.hetzner.com/cloud/firewalls/overview#limits). This
    * limit applies to [Servers](#tag/servers) applied via a matching [Label
    * Selector](#description/label-selector) as well. Updates to resources
    * matching or no longer matching a [Label
    * Selector](#description/label-selector) can take up to a few seconds to be
    * processed. A [Firewall](#tag/firewalls) is applied to a resource once the
    * related [Action](#tag/actions) with command `apply_firewall` successfully
    * finished. #### Operation specific errors | Status | Code | Description | |
    * --- | --- | --- | | `404` | `firewall_resource_not_found` | The resource
    * the [Firewall](#tag/firewalls) should be applied to was not found | |
    * `422` | `firewall_already_applied` | [Firewall](#tag/firewalls) is already
    * applied to resource | | `422` | `incompatible_network_type` | The network
    * type of the resource is not supported by [Firewalls](#tag/firewalls) | |
    * `422` | `private_net_only_server` | The [Server](#tag/servers) the
    * [Firewall](#tag/firewalls) should be applied to has no public interface |
    *
    * Expected answers: code 201 : ActionListResponse (Request succeeded.) code
    * 4xx : GetActions4xxResponse (Request failed with a user error.) code 5xx :
  ...
  /** Returns a specific [Action](#tag/actions) for a
    ...
    *   ID of the Action.
    */
  def getFirewallAction(id: Long, actionId: Long)(using
      Auth <:< hcloud.Authorization.BearerToken
  ...
  /** Returns the specific [Action](#tag/actions).
    ...
    *   ID of the Action.
    */
  def getFirewallsAction(id: Long)(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[Either[ResponseException[String], ActionResponse]] =
    val idPathParam =
      PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
  ...
  /** Returns all [Actions](#tag/actions) for [Firewalls](#tag/firewalls). Use
    * the provided URI parameters to modify the result.
    *
    * Expected answers: code 200 : ActionListResponseWithMeta (Request
    * succeeded.) code 4xx : GetActions4xxResponse (Request failed with a user
    * error.) code 5xx : GetActions5xxResponse (Request failed with a server
  ...
  /** Removes a [Firewall](#tag/firewalls) from multiple resources. Supported
    * resources: - [Servers](#tag/servers) (with a public network interface) A
    * [Firewall](#tag/firewalls) is removed from a resource once the related
    * [Action](#tag/actions) with command `remove_firewall` successfully
    * finished. #### Operation specific errors | Status | Code | Description | |
    * --- | --- | --- | | `404` | `firewall_resource_not_found` | The resource
    * the [Firewall](#tag/firewalls) should be removed from was not found | |
    * `422` | `firewall_managed_by_label_selector` | [Firewall](#tag/firewall)
    * is applied via a [Label Selector](#description/label-selector) and cannot
    * be removed manually |
    *
    * Expected answers: code 201 : ActionListResponse (Request succeeded.) code
    * 4xx : GetActions4xxResponse (Request failed with a user error.) code 5xx :
    ...
    */
  def removeFirewallFromResources(
      id: Long,
  ...
  /** Set the rules of a [Firewall](#tag/firewalls). Overwrites the existing
    * rules with the given ones. Pass an empty array to remove all rules. Rules
    * are limited to 50 entries per [Firewall](#tag/firewalls) and [500
    * effective
    ...
    */
  def setFirewallRules(id: Long, setRulesRequest: SetRulesRequest)(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[
    Either[ResponseException[String], ActionListResponse]
  ] =
