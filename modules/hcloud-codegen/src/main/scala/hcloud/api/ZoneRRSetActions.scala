...
import hcloud.models.RRSetProtection
import hcloud.models.RemoveZoneRrsetRecordsRequest
import hcloud.models.SetZoneRrsetRecordsRequest
import hcloud.models.UpdateZoneRrsetRecordsRequest
...
object ZoneRRSetActions:
  def apply(
      baseUrl: String = "https://api.hetzner.cloud/v1"
  ): ZoneRRSetActions[hcloud.Authorization.NoAuthorization.type] =
    ZoneRRSetActions(baseUrl, hcloud.Authorization.NoAuthorization)
  def withBasicAuth(
      baseUrl: String,
      username: String,
      password: String
  ): ZoneRRSetActions[hcloud.Authorization.BasicAuth] =
    ZoneRRSetActions(
      baseUrl,
  ...
  def withApiKeyAuth(
      baseUrl: String,
      apiKey: String
  ): ZoneRRSetActions[hcloud.Authorization.ApiKey] =
    ZoneRRSetActions(baseUrl, hcloud.Authorization.ApiKey(apiKey))

  def withBearerTokenAuth(
      baseUrl: String,
      token: String
  ): ZoneRRSetActions[hcloud.Authorization.BearerToken] =
    ZoneRRSetActions(baseUrl, hcloud.Authorization.BearerToken(token))

case class ZoneRRSetActions[Auth <: hcloud.Authorization] private (
    baseUrl: String,
  ...
  def withNoAuth: ZoneRRSetActions[hcloud.Authorization.NoAuthorization.type] =
  ...
  /** Adds resource records (RRs) to an [RRSet](#tag/zone-rrsets) in the
    * [Zone](#tag/zones). For convenience, the [RRSet](#tag/zone-rrsets) will be
    * automatically created if it doesn't exist. Otherwise, the new records are
    * appended to the existing [RRSet](#tag/zone-rrsets). Only applicable for
    * [Zones](#tag/zones) in primary mode. #### Operation specific errors |
    * Status | Code | Description | | --- | --- | --- | | `422` |
    * `incorrect_zone_mode` | This operation is not supported for this Zone's
    * `mode`. |
    *
  ...
  /** Changes the protection of an [RRSet](#tag/zone-rrsets) in the
    * [Zone](#tag/zones). Only applicable for [Zones](#tag/zones) in primary
    * mode. #### Operation specific errors | Status | Code | Description | | --- |
    * --- | --- | | `422` | `incorrect_zone_mode` | This operation is not
    * supported for this Zone's `mode`. |
    *
    * Expected answers: code 201 : ActionResponse1 (Request succeeded.) code 4xx
    * : GetActions4xxResponse (Request failed with a user error.) code 5xx :
    * GetActions5xxResponse (Request failed with a server error.)
  ...
  /** Changes the Time To Live (TTL) of an [RRSet](#tag/zone-rrsets) in the
    * [Zone](#tag/zones). Only applicable for [Zones](#tag/zones) in primary
    * mode. #### Operation specific errors | Status | Code | Description | | --- |
    * --- | --- | | `422` | `incorrect_zone_mode` | This operation is not
    * supported for this Zone's `mode`. |
  ...
  /** Removes resource records (RRs) from an existing [RRSet](#tag/zone-rrsets)
    * in the [Zone](#tag/zones). For convenience, the [RRSet](#tag/zone-rrsets)
    * will be automatically deleted if it doesn't contain any RRs afterwards.
    * Only applicable for [Zones](#tag/zones) in primary mode. #### Operation
    * specific errors | Status | Code | Description | | --- | --- | --- | |
    * `422` | `incorrect_zone_mode` | This operation is not supported for this
    * Zone's `mode`. |
    *
    * Expected answers: code 201 : ActionResponse1 (Request succeeded.) code 4xx
    * : GetActions4xxResponse (Request failed with a user error.) code 5xx :
  ...
  /** Overwrites the resource records (RRs) of an existing
    * [RRSet](#tag/zone-rrsets) in the [Zone](#tag/zones). Only applicable for
    * [Zones](#tag/zones) in primary mode. #### Operation specific errors |
    * Status | Code | Description | | --- | --- | --- | | `422` |
    * `incorrect_zone_mode` | This operation is not supported for this Zone's
    * `mode`. |
  ...
  /** Updates resource records' (RRs) comments of an existing
    * [RRSet](#tag/zone-rrsets) in the [Zone](#tag/zones). Only applicable for
    * [Zones](#tag/zones) in primary mode. #### Operation specific errors |
    * Status | Code | Description | | --- | --- | --- | | `422` |
    * `incorrect_zone_mode` | This operation is not supported for this Zone's
    * `mode`. |
    *
    * Expected answers: code 200 : ActionResponse1 (Request succeeded.) code 4xx
    * : GetActions4xxResponse (Request failed with a user error.) code 5xx :
    * GetActions5xxResponse (Request failed with a server error.)
    *
    * Available security schemes: APIToken (http)
