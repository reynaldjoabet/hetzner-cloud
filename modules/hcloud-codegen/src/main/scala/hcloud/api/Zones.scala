...
case class Zones[Auth <: hcloud.Authorization] private (
  ...
  /** Creates a [Zone](#tag/zones). A default `SOA` and three `NS` resource
    * records with the assigned Hetzner nameservers are created automatically.
    *
    * Expected answers: code 201 : CreateZone201Response (Request succeeded.)
  ...
  def createZone(createZoneRequest: CreateZoneRequest)(using
    ...
    Either[ResponseException[String], CreateZone201Response]
  ] =
    val requestURL =
      uri"$baseUrl/zones"
  ...
  /** Returns a single [Zone](#tag/zones).
    *
    * Expected answers: code 200 : GetZone200Response (Request succeeded.) code
  ...
  def getZone(idOrName: String)(using
    ...
    Either[ResponseException[String], GetZone200Response]
  ] =
    val idOrNamePathParam = PathSerializable.serialize(
      "id_or_name",
      idOrName,
  ...
  /** Returns a generated [Zone](#tag/zones) file in BIND (RFC
    * [1034](https://datatracker.ietf.org/doc/html/rfc1034)/[1035](https://datatracker.ietf.org/doc/html/rfc1035))
    * format. Only applicable for [Zones](#tag/zones) in primary mode. ####
    * Operation specific errors | Status | Code | Description | | --- | --- |
    * --- | | `422` | `incorrect_zone_mode` | This operation is not supported
    * for this Zone's `mode`. |
    *
    * Expected answers: code 200 : GetZoneZonefile200Response (Request
    ...
    *   ID or Name of the Zone.
    */
  def getZoneZonefile(idOrName: String)(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[
    Either[ResponseException[String], GetZoneZonefile200Response]
  ] =
    val idOrNamePathParam = PathSerializable.serialize(
      "id_or_name",
      idOrName,
  ...
  /** Returns all [Zones](#tag/zones). Use the provided URI parameters to modify
    * the result.
    *
    * Expected answers: code 200 : ListZones200Response (Request succeeded.)
    * code 4xx : GetActions4xxResponse (Request failed with a user error.) code
    * 5xx : GetActions5xxResponse (Request failed with a server error.)
    *
    * Available security schemes: APIToken (http)
  ...
  def listZones(
    ...
    Either[ResponseException[String], ListZones200Response]
  ] =
    val requestURL =
      uri"$baseUrl/zones"
        .addParams(
  ...
  /** Updates a [Zone](#tag/zones). To modify resource record sets
    * ([RRSets](#tag/zone-rrsets)), use the [RRSet Actions
    ...
    */
  def updateZone(idOrName: String, zoneUpdateRequest: ZoneUpdateRequest)(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[
    Either[ResponseException[String], GetZone200Response]
  ] =
