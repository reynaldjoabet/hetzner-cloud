...
case class ISOs[Auth <: hcloud.Authorization] private (baseUrl: String, authConfig: hcloud.Authorization):
   ...
   * 
   * Expected answers:
   *   code 200 : GetIso200Response (Request succeeded.)
   ...
   */
  def getIso(id: Long)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], GetIso200Response]] =
   ...
   * 
   * Expected answers:
   *   code 200 : ListIsos200Response (Request succeeded.)
   *   code 4xx : GetActions4xxResponse (Request failed with a user error.)
   *   code 5xx : GetActions5xxResponse (Request failed with a server error.)
   * 
   * Available security schemes:
   ...
   */
  def listIsos(name: Option[String] = scala.None, architecture: Option[String] = scala.None, includeArchitectureWildcard: Option[Boolean] = scala.None, page: Option[Long] = scala.None, perPage: Option[Long] = scala.None)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], ListIsos200Response]] =
    val requestURL =
      uri"$baseUrl/isos"
