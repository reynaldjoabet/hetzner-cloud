...
case class PrimaryIPs[Auth <: hcloud.Authorization] private (
  ...
  /** Create a new [Primary IP](#tag/primary-ips). Can optionally be assigned to
    * a resource by providing an `assignee_id` and `assignee_type`. If not
    * assigned to a resource the `location` key needs to be provided. This can
    * be either the ID or the name of the [Location](#tag/locations) this
    * [Primary IP](#tag/primary-ips) shall be created in. A [Primary
    * IP](#tag/primary-ips) can only be assigned to resource in the same
    * [Location](#tag/locations) later on. The `datacenter` key is deprecated in
    * favor of `location` and will be removed after 01 July 2026. #### Operation
    * specific errors | Status | Code | Description | | --- | --- | --- | |
    * `422` | `server_not_stopped` | The specified [Server](#tag/servers) is
    * running, but needs to be powered off | | `422` | `server_has_ipv4` | The
    * [Server](#tag/servers) already has an ipv4 address | | `422` |
    * `server_has_ipv6` | The [Server](#tag/servers) already has an ipv6 address |
    *
    * Expected answers: code 201 : CreatePrimaryIPResponse (Request succeeded.)
    ...
    *
    * @param primaryIPCreateRequest
    *   Request Body for creating a new [Primary IP](#tag/primary-ips). The
    *   `location`, `datacenter` and `assignee_id`/`assignee_type` attributes
    *   are mutually exclusive.
    */
  def createPrimaryIp(primaryIPCreateRequest: PrimaryIPCreateRequest)(using
  ...
  /** Deletes a [Primary IP](#tag/primary-ips). The [Server](#tag/servers) must
    * be powered off (status `off`) in order for this operation to succeed. If
    * assigned to a [Server](#tag/servers) the [Primary IP](#tag/primary-ips)
    * will be unassigned automatically until 1 May 2026. After this date, the
    * [Primary IP](#tag/primary-ips) needs to be unassigned before it can be
    * deleted. #### Operation specific errors | Status | Code | Description | |
    * --- | --- | --- | | `422` | `must_be_unassigned` | Error when IP is still
    * assigned to a Resource. This error will appear as of 1 May 2026. |
    *
    * Expected answers: code 204 : (Request succeeded.) code 4xx :
  ...
  /** Returns a [Primary IP](#tag/primary-ips).
    *
    * Expected answers: code 200 : PrimaryIPResponse (Request succeeded.) code
  ...
  def getPrimaryIp(id: Long)(using
    ...
    Either[ResponseException[String], PrimaryIPResponse]
  ] =
    val idPathParam =
      PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
    val requestURL =
  ...
  /** List multiple [Primary IPs](#tag/primary-ips). Use the provided URI
    * parameters to modify the result.
    *
    * Expected answers: code 200 : PrimaryIPsResponse (Request succeeded.) code
    * 4xx : GetActions4xxResponse (Request failed with a user error.) code 5xx :
    * GetActions5xxResponse (Request failed with a server error.)
    *
    * Available security schemes: APIToken (http)
  ...
  def listPrimaryIps(
    ...
    Either[ResponseException[String], PrimaryIPsResponse]
  ] =
    val requestURL =
      uri"$baseUrl/primary_ips"
        .addParams(
  ...
  /** Update a [Primary IP](#tag/primary-ips). If another change is concurrently
    * performed on this [Primary IP](#tag/primary-ips), a error response with
    ...
    */
  def updatePrimaryIp(id: Long, primaryIPUpdateRequest: PrimaryIPUpdateRequest)(
