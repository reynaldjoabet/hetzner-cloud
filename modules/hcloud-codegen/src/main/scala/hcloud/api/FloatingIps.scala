...
import hcloud.models.CreateFloatingIp201Response
import hcloud.models.FloatingIPCreateRequest
import hcloud.models.FloatingIPUpdateRequest
import hcloud.models.GetActions4xxResponse
import hcloud.models.GetActions5xxResponse
import hcloud.models.GetFloatingIp200Response
import hcloud.models.ListFloatingIps200Response
...
object FloatingIPs:
  ...
  ): FloatingIPs[hcloud.Authorization.NoAuthorization.type] =
    FloatingIPs(baseUrl, hcloud.Authorization.NoAuthorization)
  def withBasicAuth(
      baseUrl: String,
      username: String,
      password: String
  ): FloatingIPs[hcloud.Authorization.BasicAuth] =
    FloatingIPs(baseUrl, hcloud.Authorization.BasicAuth(username, password))

  def withApiKeyAuth(
      baseUrl: String,
      apiKey: String
  ): FloatingIPs[hcloud.Authorization.ApiKey] =
    FloatingIPs(baseUrl, hcloud.Authorization.ApiKey(apiKey))

  def withBearerTokenAuth(
      baseUrl: String,
      token: String
  ): FloatingIPs[hcloud.Authorization.BearerToken] =
    FloatingIPs(baseUrl, hcloud.Authorization.BearerToken(token))

case class FloatingIPs[Auth <: hcloud.Authorization] private (
    baseUrl: String,
  ...
  /** Create a [Floating IP](#tag/floating-ips). Provide the `server` attribute
    * to assign the [Floating IP](#tag/floating-ips) to that server or provide a
    * `home_location` to locate the [Floating IP](#tag/floating-ips) at. Note
    * that the [Floating IP](#tag/floating-ips) can be assigned to a
    * [Server](#tag/servers) in any [Location](#tag/locations) later on. For
    * optimal routing it is advised to use the [Floating IP](#tag/floating-ips)
    * in the same [Location](#tag/locations) it was created in.
    *
    * Expected answers: code 201 : CreateFloatingIp201Response (Request
    ...
    *
    * @param floatingIPCreateRequest
    *   The `type` argument is required while `home_location` and `server` are
    *   mutually exclusive.
    */
  def createFloatingIp(floatingIPCreateRequest: FloatingIPCreateRequest)(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[
    Either[ResponseException[String], CreateFloatingIp201Response]
  ] =
    val requestURL =
    ...
    basicRequest
      ...
      .auth(authConfig)
      .body(asJson(floatingIPCreateRequest))
      .response(asJson[CreateFloatingIp201Response])

  /** Deletes a [Floating IP](#tag/floating-ips). If assigned to a
    * [Server](#tag/servers) the [Floating IP](#tag/floating-ips) will be
    * unassigned automatically until 1 May 2026. After this date, the [Floating
    * IP](#tag/floating-ips) needs to be unassigned before it can be deleted.
    * #### Operation specific errors | Status | Code | Description | | --- | --- |
    * --- | | `422` | `must_be_unassigned` | Error when IP is still assigned to
    * a Resource. This error will appear as of 1 May 2026. |
    *
    ...
    */
  def deleteFloatingIp(id: Long)(using
      Auth <:< hcloud.Authorization.BearerToken
  ...
  /** Returns a single [Floating IP](#tag/floating-ips).
    *
    * Expected answers: code 200 : GetFloatingIp200Response (Request succeeded.)
    ...
    *   ID of the Floating IP.
    */
  def getFloatingIp(id: Long)(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[
    Either[ResponseException[String], GetFloatingIp200Response]
  ] =
    val idPathParam =
      PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
    val requestURL =
    ...
    basicRequest
      ...
      .contentType("application/json")
      .auth(authConfig)
      .response(asJson[GetFloatingIp200Response])

  /** List multiple [Floating IPs](#tag/floating-ips). Use the provided URI
    * parameters to modify the result.
    *
    * Expected answers: code 200 : ListFloatingIps200Response (Request
    * succeeded.) code 4xx : GetActions4xxResponse (Request failed with a user
    * error.) code 5xx : GetActions5xxResponse (Request failed with a server
    * error.)
    *
  ...
  def listFloatingIps(
    ...
    Either[ResponseException[String], ListFloatingIps200Response]
  ] =
    val requestURL =
      uri"$baseUrl/floating_ips"
        .addParams(
    ...
    basicRequest
      ...
      .contentType("application/json")
      .auth(authConfig)
      .response(asJson[ListFloatingIps200Response])
  ...
  def updateFloatingIp(
    ...
    basicRequest
      ...
      .contentType("application/json")
      .auth(authConfig)
      .body(asJson(floatingIPUpdateRequest))
      .response(asJson[GetFloatingIp200Response])
