...
import hcloud.models.GetPricing200Response
...
case class Pricing[Auth <: hcloud.Authorization] private (
  ...
  /** Returns prices for all resources available on the platform. VAT and
    ...
    *
    * Expected answers: code 200 : GetPricing200Response (Request succeeded.)
    ...
    */
  def getPricing(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[
    Either[ResponseException[String], GetPricing200Response]
  ] =
    val requestURL =
      uri"$baseUrl/pricing"
    ...
    basicRequest
      ...
      .auth(authConfig)
      .response(asJson[GetPricing200Response])
