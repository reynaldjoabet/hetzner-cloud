...
import hcloud.models.GetImage200Response
import hcloud.models.ListImages200Response
import hcloud.models.UpdateImageRequest
...
object Images:
  def apply(baseUrl: String = "https://api.hetzner.cloud/v1"): Images[hcloud.Authorization.NoAuthorization.type] = Images(baseUrl, hcloud.Authorization.NoAuthorization)
  def withBasicAuth(baseUrl: String, username: String, password: String): Images[hcloud.Authorization.BasicAuth] =
    Images(baseUrl, hcloud.Authorization.BasicAuth(username, password))

  def withApiKeyAuth(baseUrl: String, apiKey: String): Images[hcloud.Authorization.ApiKey] =
    Images(baseUrl, hcloud.Authorization.ApiKey(apiKey))

  def withBearerTokenAuth(baseUrl: String, token: String): Images[hcloud.Authorization.BearerToken] =
    Images(baseUrl, hcloud.Authorization.BearerToken(token))

case class Images[Auth <: hcloud.Authorization] private (baseUrl: String, authConfig: hcloud.Authorization):
  def withBasicAuth(username: String, password: String): Images[hcloud.Authorization.BasicAuth] =
  ...
  def withBearerTokenAuth(token: String): Images[hcloud.Authorization.BearerToken] =
   ...
   */
  def deleteImage(id: Long)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], Unit]] =
   ...
   * 
   * Expected answers:
   *   code 200 : GetImage200Response (Request succeeded.)
   *   code 4xx : GetActions4xxResponse (Request failed with a user error.)
   *   code 5xx : GetActions5xxResponse (Request failed with a server error.)
   * 
   * Available security schemes:
   ...
   * @param id ID of the Image.
   */
  def getImage(id: Long)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], GetImage200Response]] =
    val idPathParam = PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
    val requestURL =
      uri"$baseUrl/images/${idPathParam}"
    ...
    basicRequest
      ...
      .contentType("application/json")
      .auth(authConfig)
      .response(asJson[GetImage200Response])

  /**
   * Returns all Image objects. You can select specific Image types only and sort the results by using URI parameters. 
   * 
   * Expected answers:
   *   code 200 : ListImages200Response (Request succeeded.)
   *   code 4xx : GetActions4xxResponse (Request failed with a user error.)
   *   code 5xx : GetActions5xxResponse (Request failed with a server error.)
   * 
   * Available security schemes:
   ...
   * @param `type` Filter resources by type. May be used multiple times.  The response will only contain the resources with the specified type. 
   ...
   * @param boundTo Filter Images by their linked Server ID. May be used multiple times.  Only available for Images of type `backup`. 
   ...
   */
  def listImages(sort: Seq[String], `type`: Seq[String], status: Seq[String], boundTo: Seq[String], includeDeprecated: Option[Boolean] = scala.None, name: Option[String] = scala.None, labelSelector: Option[String] = scala.None, architecture: Option[String] = scala.None, page: Option[Long] = scala.None, perPage: Option[Long] = scala.None)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], ListImages200Response]] =
    val requestURL =
      uri"$baseUrl/images"
        ...
        .addParams(FormSerializable.serialize("bound_to", boundTo, FormStyleFormat.FORM, true): _*)
        .addParams(FormSerializable.serialize("include_deprecated", includeDeprecated, FormStyleFormat.FORM, true): _*)
    ...
    basicRequest
      ...
      .contentType("application/json")
      .auth(authConfig)
      .response(asJson[ListImages200Response])
   ...
   */
  def updateImage(id: Long, updateImageRequest: UpdateImageRequest)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], GetImage200Response]] =
    val idPathParam = PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
    val requestURL =
      uri"$baseUrl/images/${idPathParam}"
    ...
    basicRequest
      ...
      .contentType("application/json")
      .auth(authConfig)
      .body(asJson(updateImageRequest))
      .response(asJson[GetImage200Response])
