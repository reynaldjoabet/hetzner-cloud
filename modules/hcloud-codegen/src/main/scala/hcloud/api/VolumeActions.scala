...
object VolumeActions:
  ...
  ): VolumeActions[hcloud.Authorization.NoAuthorization.type] =
    VolumeActions(baseUrl, hcloud.Authorization.NoAuthorization)
  def withBasicAuth(
      baseUrl: String,
      username: String,
      password: String
  ): VolumeActions[hcloud.Authorization.BasicAuth] =
    VolumeActions(baseUrl, hcloud.Authorization.BasicAuth(username, password))

  def withApiKeyAuth(
      baseUrl: String,
      apiKey: String
  ): VolumeActions[hcloud.Authorization.ApiKey] =
    VolumeActions(baseUrl, hcloud.Authorization.ApiKey(apiKey))

  def withBearerTokenAuth(
      baseUrl: String,
      token: String
  ): VolumeActions[hcloud.Authorization.BearerToken] =
    VolumeActions(baseUrl, hcloud.Authorization.BearerToken(token))

case class VolumeActions[Auth <: hcloud.Authorization] private (
    baseUrl: String,
  ...
  /** Attaches a Volume to a Server. Works only if the Server is in the same
    ...
    */
  def attachVolume(id: Long, attachVolumeRequest: AttachVolumeRequest)(using
  ...
  /** Returns a specific Action for a Volume.
    ...
    *   ID of the Action.
    */
  def getVolumeAction(id: Long, actionId: Long)(using
      Auth <:< hcloud.Authorization.BearerToken
  ...
  /** Returns a specific Action object.
    ...
    *   ID of the Action.
    */
  def getVolumesAction(id: Long)(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[Either[ResponseException[String], ActionResponse]] =
    val idPathParam =
      PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
