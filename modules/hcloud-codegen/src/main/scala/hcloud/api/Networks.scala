...
case class Networks[Auth <: hcloud.Authorization] private (
  ...
  /** Creates a [Network](#tag/networks). The provided `ip_range` can only be
    * extended later on, but not reduced. Subnets can be added now or later on
    * using the [add subnet action](#tag/network-actions/add_network_subnet). If
    * you do not specify an `ip_range` for the subnet the first available /24
    * range will be used. Routes can be added now or later by using the [add
    * route action](#tag/network-actions/add_network_route).
    *
    * Expected answers: code 201 : CreateNetwork201Response (Request succeeded.)
    ...
    */
  def createNetwork(networkCreateRequest: NetworkCreateRequest)(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[
    Either[ResponseException[String], CreateNetwork201Response]
  ] =
    val requestURL =
  ...
  /** Deletes a [Network](#tag/networks). Attached resources will be detached
    * automatically.
    *
  ...
  /** Get a specific [Network](#tag/networks).
    *
    * Expected answers: code 200 : CreateNetwork201Response (Request succeeded.)
    * code 4xx : GetActions4xxResponse (Request failed with a user error.) code
    * 5xx : GetActions5xxResponse (Request failed with a server error.)
    *
    * Available security schemes: APIToken (http)
  ...
  /** List multiple [Networks](#tag/networks). Use the provided URI parameters
    * to modify the result.
    *
    * Expected answers: code 200 : ListNetworks200Response (Request succeeded.)
    * code 4xx : GetActions4xxResponse (Request failed with a user error.) code
    * 5xx : GetActions5xxResponse (Request failed with a server error.)
    *
    * Available security schemes: APIToken (http)
  ...
  def listNetworks(
    ...
    Either[ResponseException[String], ListNetworks200Response]
  ] =
    val requestURL =
      uri"$baseUrl/networks"
        .addParams(
  ...
  /** Update a [Network](#tag/networks). If a change is currently being
    * performed on this [Network](#tag/networks), a error response with code
    ...
    */
  def updateNetwork(id: Long, networkUpdateRequest: NetworkUpdateRequest)(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[
    Either[ResponseException[String], CreateNetwork201Response]
  ] =
