...
case class FloatingIPs[Auth <: hcloud.Authorization] private (baseUrl: String, authConfig: hcloud.Authorization):
   ...
   * 
   * Expected answers:
   *   code 201 : CreateFloatingIp201Response (Request succeeded.)
   ...
   *   APIToken (http)
   * 
   * @param floatingIPCreateRequest The `type` argument is required while `home_location` and `server` are mutually exclusive.
   */
  def createFloatingIp(floatingIPCreateRequest: FloatingIPCreateRequest)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], CreateFloatingIp201Response]] =
    val requestURL =
   ...
   * 
   * Expected answers:
   *   code 200 : GetFloatingIp200Response (Request succeeded.)
   *   code 4xx : GetActions4xxResponse (Request failed with a user error.)
   *   code 5xx : GetActions5xxResponse (Request failed with a server error.)
   * 
   * Available security schemes:
   ...
   * @param id ID of the Floating IP.
   */
  def getFloatingIp(id: Long)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], GetFloatingIp200Response]] =
    val idPathParam = PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
    val requestURL =
      uri"$baseUrl/floating_ips/${idPathParam}"
   ...
   * 
   * Expected answers:
   *   code 200 : ListFloatingIps200Response (Request succeeded.)
   *   code 4xx : GetActions4xxResponse (Request failed with a user error.)
   *   code 5xx : GetActions5xxResponse (Request failed with a server error.)
   * 
   * Available security schemes:
   ...
   */
  def listFloatingIps(name: Option[String] = scala.None, labelSelector: Option[String] = scala.None, sort: Seq[String], page: Option[Long] = scala.None, perPage: Option[Long] = scala.None)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], ListFloatingIps200Response]] =
    val requestURL =
      uri"$baseUrl/floating_ips"
   ...
   */
  def updateFloatingIp(id: Long, floatingIPUpdateRequest: FloatingIPUpdateRequest)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], GetFloatingIp200Response]] =
    val idPathParam = PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
    val requestURL =
      uri"$baseUrl/floating_ips/${idPathParam}"
