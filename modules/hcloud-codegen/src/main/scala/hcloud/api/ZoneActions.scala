...
object ZoneActions:
  ...
  ): ZoneActions[hcloud.Authorization.NoAuthorization.type] =
    ZoneActions(baseUrl, hcloud.Authorization.NoAuthorization)
  def withBasicAuth(
      baseUrl: String,
      username: String,
      password: String
  ): ZoneActions[hcloud.Authorization.BasicAuth] =
    ZoneActions(baseUrl, hcloud.Authorization.BasicAuth(username, password))

  def withApiKeyAuth(
      baseUrl: String,
      apiKey: String
  ): ZoneActions[hcloud.Authorization.ApiKey] =
    ZoneActions(baseUrl, hcloud.Authorization.ApiKey(apiKey))

  def withBearerTokenAuth(
      baseUrl: String,
      token: String
  ): ZoneActions[hcloud.Authorization.BearerToken] =
    ZoneActions(baseUrl, hcloud.Authorization.BearerToken(token))

case class ZoneActions[Auth <: hcloud.Authorization] private (
    baseUrl: String,
  ...
  /** Overwrites the primary nameservers of a [Zone](#tag/zones). Only
    * applicable for [Zones](#tag/zones) in secondary mode. #### Operation
    ...
    */
  def changeZonePrimaryNameservers(
      idOrName: String,
  ...
  /** Changes the default Time To Live (TTL) of a [Zone](#tag/zones). This TTL
    * is used for [RRSets](#tag/zone-rrsets) that do not explicitly define a
    * TTL. Only applicable for [Zones](#tag/zones) in primary mode. ####
  ...
  /** Returns a specific [Action](#tag/actions) for a [Zone](#tag/zones).
    ...
    *   ID of the Action.
    */
  def getZoneAction(idOrName: String, actionId: Long)(using
      Auth <:< hcloud.Authorization.BearerToken
  ...
  /** Returns a specific [Action](#tag/actions).
    ...
    *   ID of the Action.
    */
  def getZonesAction(id: Long)(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[Either[ResponseException[String], ActionResponse]] =
    val idPathParam =
  ...
  /** Imports a zone file, replacing all resource record sets
    * ([RRSets](#tag/zone-rrsets)). The import will fail if existing
    * [RRSet](#tag/zone-rrsets) are `change` protected. See [Zone file
    * import](#tag/zones/zone-file-import) for more details. Only applicable for
  ...
  /** Returns all [Actions](#tag/actions) for a [Zone](#tag/zones). Use the
  ...
  /** Returns all [Zone](#tag/zones) [Actions](#tag/actions). Use the provided
    * URI parameters to modify the result.
    *
    * Expected answers: code 200 : ActionListResponseWithMeta (Request
    * succeeded.) code 4xx : GetActions4xxResponse (Request failed with a user
    * error.) code 5xx : GetActions5xxResponse (Request failed with a server
