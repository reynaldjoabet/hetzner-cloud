...
import hcloud.models.CreateZoneRequestRrsetsInner
import hcloud.models.CreateZoneRrset201Response
import hcloud.models.GetActions4xxResponse
import hcloud.models.GetActions5xxResponse
import hcloud.models.GetZoneRrset200Response
import hcloud.models.ListZoneRrsets200Response
import hcloud.models.UpdateZoneRrsetRequest
...
object ZoneRRSets:
  def apply(baseUrl: String = "https://api.hetzner.cloud/v1"): ZoneRRSets[hcloud.Authorization.NoAuthorization.type] = ZoneRRSets(baseUrl, hcloud.Authorization.NoAuthorization)
  def withBasicAuth(baseUrl: String, username: String, password: String): ZoneRRSets[hcloud.Authorization.BasicAuth] =
    ZoneRRSets(baseUrl, hcloud.Authorization.BasicAuth(username, password))

  def withApiKeyAuth(baseUrl: String, apiKey: String): ZoneRRSets[hcloud.Authorization.ApiKey] =
    ZoneRRSets(baseUrl, hcloud.Authorization.ApiKey(apiKey))

  def withBearerTokenAuth(baseUrl: String, token: String): ZoneRRSets[hcloud.Authorization.BearerToken] =
    ZoneRRSets(baseUrl, hcloud.Authorization.BearerToken(token))

case class ZoneRRSets[Auth <: hcloud.Authorization] private (baseUrl: String, authConfig: hcloud.Authorization):
  def withBasicAuth(username: String, password: String): ZoneRRSets[hcloud.Authorization.BasicAuth] =
    copy(authConfig = hcloud.Authorization.BasicAuth(username, password))

  def withApiKeyAuth(apiKey: String): ZoneRRSets[hcloud.Authorization.ApiKey] =
    copy(authConfig = hcloud.Authorization.ApiKey(apiKey))

  def withNoAuth: ZoneRRSets[hcloud.Authorization.NoAuthorization.type] =
    copy(authConfig = hcloud.Authorization.NoAuthorization)

  def withBearerTokenAuth(token: String): ZoneRRSets[hcloud.Authorization.BearerToken] =
   ...
   * 
   * Expected answers:
   *   code 201 : CreateZoneRrset201Response (Request succeeded.)
   ...
   * @param createZoneRequestRrsetsInner 
   */
  def createZoneRrset(idOrName: String, createZoneRequestRrsetsInner: CreateZoneRequestRrsetsInner)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], CreateZoneRrset201Response]] =
    ...
    basicRequest
      ...
      .auth(authConfig)
      .body(asJson(createZoneRequestRrsetsInner))
      .response(asJson[CreateZoneRrset201Response])
   ...
   * @param rrType 
   */
  def deleteZoneRrset(idOrName: String, rrName: String, rrType: String)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], ActionResponse]] =
    val idOrNamePathParam = PathSerializable.serialize("id_or_name", idOrName, PathStyleFormat.SIMPLE, false)
   ...
   * 
   * Expected answers:
   *   code 200 : GetZoneRrset200Response (Request succeeded.)
   *   code 4xx : GetActions4xxResponse (Request failed with a user error.)
   *   code 5xx : GetActions5xxResponse (Request failed with a server error.)
   * 
   * Available security schemes:
   ...
   * @param rrType 
   */
  def getZoneRrset(idOrName: String, rrName: String, rrType: String)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], GetZoneRrset200Response]] =
    val idOrNamePathParam = PathSerializable.serialize("id_or_name", idOrName, PathStyleFormat.SIMPLE, false)
    val rrNamePathParam = PathSerializable.serialize("rr_name", rrName, PathStyleFormat.SIMPLE, false)
    val rrTypePathParam = PathSerializable.serialize("rr_type", rrType, PathStyleFormat.SIMPLE, false)
    val requestURL =
    ...
    basicRequest
      ...
      .contentType("application/json")
      .auth(authConfig)
      .response(asJson[GetZoneRrset200Response])
   ...
   * 
   * Expected answers:
   *   code 200 : ListZoneRrsets200Response (Request succeeded.)
   *   code 4xx : GetActions4xxResponse (Request failed with a user error.)
   *   code 5xx : GetActions5xxResponse (Request failed with a server error.)
   * 
   * Available security schemes:
   ...
   * @param `type` Filter resources by their type. May be used multiple times.  The response will only contain resources matching the specified types. 
   * @param labelSelector Filter resources by labels.  The response will only contain resources matching the label selector. For more information, see \"[Label Selector](#description/label-selector)\". 
   * @param sort Sort resources by field and direction. May be used multiple times.  For more information, see \"[Sorting](#description/sorting)\". 
   ...
   */
  def listZoneRrsets(idOrName: String, name: Option[String] = scala.None, `type`: Seq[String], labelSelector: Option[String] = scala.None, sort: Seq[String], page: Option[Long] = scala.None, perPage: Option[Long] = scala.None)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], ListZoneRrsets200Response]] =
    val idOrNamePathParam = PathSerializable.serialize("id_or_name", idOrName, PathStyleFormat.SIMPLE, false)
    val requestURL =
      uri"$baseUrl/zones/${idOrNamePathParam}/rrsets"
    ...
    basicRequest
      ...
      .contentType("application/json")
      .auth(authConfig)
      .response(asJson[ListZoneRrsets200Response])
   ...
   * @param rrName 
   * @param rrType 
   * @param updateZoneRrsetRequest 
   */
  def updateZoneRrset(idOrName: String, rrName: String, rrType: String, updateZoneRrsetRequest: UpdateZoneRrsetRequest)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], GetZoneRrset200Response]] =
    val idOrNamePathParam = PathSerializable.serialize("id_or_name", idOrName, PathStyleFormat.SIMPLE, false)
    val rrNamePathParam = PathSerializable.serialize("rr_name", rrName, PathStyleFormat.SIMPLE, false)
    val rrTypePathParam = PathSerializable.serialize("rr_type", rrType, PathStyleFormat.SIMPLE, false)
    val requestURL =
    ...
    basicRequest
      ...
      .contentType("application/json")
      .auth(authConfig)
      .body(asJson(updateZoneRrsetRequest))
      .response(asJson[GetZoneRrset200Response])
