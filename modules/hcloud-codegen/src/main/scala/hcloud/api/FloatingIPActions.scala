...
import hcloud.models.ListFloatingIpActions200Response
...
object FloatingIPActions:
  def apply(baseUrl: String = "https://api.hetzner.cloud/v1"): FloatingIPActions[hcloud.Authorization.NoAuthorization.type] = FloatingIPActions(baseUrl, hcloud.Authorization.NoAuthorization)
  def withBasicAuth(baseUrl: String, username: String, password: String): FloatingIPActions[hcloud.Authorization.BasicAuth] =
    FloatingIPActions(baseUrl, hcloud.Authorization.BasicAuth(username, password))

  def withApiKeyAuth(baseUrl: String, apiKey: String): FloatingIPActions[hcloud.Authorization.ApiKey] =
    FloatingIPActions(baseUrl, hcloud.Authorization.ApiKey(apiKey))

  def withBearerTokenAuth(baseUrl: String, token: String): FloatingIPActions[hcloud.Authorization.BearerToken] =
    FloatingIPActions(baseUrl, hcloud.Authorization.BearerToken(token))

case class FloatingIPActions[Auth <: hcloud.Authorization] private (baseUrl: String, authConfig: hcloud.Authorization):
  def withBasicAuth(username: String, password: String): FloatingIPActions[hcloud.Authorization.BasicAuth] =
    copy(authConfig = hcloud.Authorization.BasicAuth(username, password))

  def withApiKeyAuth(apiKey: String): FloatingIPActions[hcloud.Authorization.ApiKey] =
    copy(authConfig = hcloud.Authorization.ApiKey(apiKey))

  def withNoAuth: FloatingIPActions[hcloud.Authorization.NoAuthorization.type] =
    copy(authConfig = hcloud.Authorization.NoAuthorization)

  def withBearerTokenAuth(token: String): FloatingIPActions[hcloud.Authorization.BearerToken] =
   ...
   */
  def getFloatingIpAction(id: Long, actionId: Long)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], ActionResponse]] =
    val idPathParam = PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
   ...
   */
  def getFloatingIpsAction(id: Long)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], ActionResponse]] =
    val idPathParam = PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
    val requestURL =
   ...
   * 
   * Expected answers:
   *   code 200 : ListFloatingIpActions200Response (Request succeeded.)
   *   code 4xx : GetActions4xxResponse (Request failed with a user error.)
   *   code 5xx : GetActions5xxResponse (Request failed with a server error.)
   * 
   * Available security schemes:
   ...
   */
  def listFloatingIpActions(id: Long, sort: Seq[String], status: Seq[String], page: Option[Long] = scala.None, perPage: Option[Long] = scala.None)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], ListFloatingIpActions200Response]] =
    val idPathParam = PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
    val requestURL =
    ...
    basicRequest
      ...
      .contentType("application/json")
      .auth(authConfig)
      .response(asJson[ListFloatingIpActions200Response])
   ...
   * @param perPage Maximum number of entries returned per page. For more information, see \"[Pagination](#description/pagination)\".
   */
  def listFloatingIpsActions(id: Seq[Long], sort: Seq[String], status: Seq[String], page: Option[Long] = scala.None, perPage: Option[Long] = scala.None)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], ActionListResponseWithMeta]] =
    val requestURL =
