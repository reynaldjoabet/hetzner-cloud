...
object FloatingIPActions:
  ...
  ): FloatingIPActions[hcloud.Authorization.NoAuthorization.type] =
    FloatingIPActions(baseUrl, hcloud.Authorization.NoAuthorization)
  def withBasicAuth(
      baseUrl: String,
      username: String,
      password: String
  ): FloatingIPActions[hcloud.Authorization.BasicAuth] =
    FloatingIPActions(
      baseUrl,
  ...
  def withApiKeyAuth(
      baseUrl: String,
      apiKey: String
  ): FloatingIPActions[hcloud.Authorization.ApiKey] =
    FloatingIPActions(baseUrl, hcloud.Authorization.ApiKey(apiKey))

  def withBearerTokenAuth(
      baseUrl: String,
      token: String
  ): FloatingIPActions[hcloud.Authorization.BearerToken] =
    FloatingIPActions(baseUrl, hcloud.Authorization.BearerToken(token))

case class FloatingIPActions[Auth <: hcloud.Authorization] private (
    baseUrl: String,
  ...
  /** Assigns a [Floating IP](#tag/floating-ips) to a [Server](#tag/servers).
    ...
    * --- | | `422` | `floating_ip_assigned` | The [Floating
    * IP](#tag/floating-ips) is already assigned |
    *
  ...
  /** Change the reverse DNS records for this [Floating IP](#tag/floating-ips).
    ...
    *   For IPv4 addresses this must be the exact address of the [Floating
    *   IP](#tag/floating-ips). For IPv6 addresses this must be a single address
    *   within the `/64` subnet of the [Floating IP](#tag/floating-ips). The
    ...
    *   [Floating IPs](#tag/floating-ips) up to 100 entries can be created.
    */
  def changeFloatingIpDnsPtr(
      id: Long,
  ...
  /** Changes the protection settings configured for the [Floating
    * IP](#tag/floating-ips).
    *
    * Expected answers: code 201 : ActionResponse1 (Request succeeded.) code 4xx
    * : GetActions4xxResponse (Request failed with a user error.) code 5xx :
  ...
  /** Returns a specific [Action](#tag/actions) for a [Floating
    * IP](#tag/floating-ips).
    *
    ...
    *   ID of the Action.
    */
  def getFloatingIpAction(id: Long, actionId: Long)(using
      Auth <:< hcloud.Authorization.BearerToken
  ...
  /** Returns a single [Action](#tag/actions).
    ...
    *   ID of the Action.
    */
  def getFloatingIpsAction(id: Long)(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[Either[ResponseException[String], ActionResponse]] =
    val idPathParam =
      PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
  ...
  /** Lists [Actions](#tag/actions) for a [Floating IP](#tag/floating-ips). Use
    * the provided URI parameters to modify the result.
    *
    * Expected answers: code 200 : ListFloatingIpActions200Response (Request
  ...
  def listFloatingIpActions(
    ...
    Either[ResponseException[String], ListFloatingIpActions200Response]
  ] =
    val idPathParam =
      PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
    val requestURL =
  ...
  /** Unassigns a [Floating IP](#tag/floating-ips). Results in the IP being
    * unreachable. Can be assigned to another resource again.
    *
    * Expected answers: code 201 : ActionResponse1 (Request succeeded.) code 4xx
    * : GetActions4xxResponse (Request failed with a user error.) code 5xx :
    * GetActions5xxResponse (Request failed with a server error.)
    ...
    *   ID of the Floating IP.
    */
  def unassignFloatingIp(id: Long)(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[Either[ResponseException[String], ActionResponse1]] =
    val idPathParam =
      PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
