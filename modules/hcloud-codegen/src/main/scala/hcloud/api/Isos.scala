...
import hcloud.models.GetIso200Response
import hcloud.models.ListIsos200Response
...
object ISOs:
  ...
  ): ISOs[hcloud.Authorization.NoAuthorization.type] =
    ISOs(baseUrl, hcloud.Authorization.NoAuthorization)
  def withBasicAuth(
      baseUrl: String,
      username: String,
      password: String
  ): ISOs[hcloud.Authorization.BasicAuth] =
    ISOs(baseUrl, hcloud.Authorization.BasicAuth(username, password))

  def withApiKeyAuth(
      baseUrl: String,
      apiKey: String
  ): ISOs[hcloud.Authorization.ApiKey] =
    ISOs(baseUrl, hcloud.Authorization.ApiKey(apiKey))

  def withBearerTokenAuth(
      baseUrl: String,
      token: String
  ): ISOs[hcloud.Authorization.BearerToken] =
    ISOs(baseUrl, hcloud.Authorization.BearerToken(token))

case class ISOs[Auth <: hcloud.Authorization] private (
    baseUrl: String,
  ...
  /** Returns a specific ISO object.
    *
    * Expected answers: code 200 : GetIso200Response (Request succeeded.) code
    ...
    *   ID of the ISO.
    */
  def getIso(id: Long)(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[
    Either[ResponseException[String], GetIso200Response]
  ] =
    val idPathParam =
    ...
    basicRequest
      ...
      .auth(authConfig)
      .response(asJson[GetIso200Response])

  /** Returns all available ISO objects.
    *
    * Expected answers: code 200 : ListIsos200Response (Request succeeded.) code
    * 4xx : GetActions4xxResponse (Request failed with a user error.) code 5xx :
    * GetActions5xxResponse (Request failed with a server error.)
    *
    * Available security schemes: APIToken (http)
    ...
    *   Architecture filter must be specified.
    * @param page
  ...
  def listIsos(
    ...
    Either[ResponseException[String], ListIsos200Response]
  ] =
    val requestURL =
      uri"$baseUrl/isos"
        .addParams(
    ...
    basicRequest
      ...
      .contentType("application/json")
      .auth(authConfig)
      .response(asJson[ListIsos200Response])
