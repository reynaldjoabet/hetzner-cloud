...
object Actions:
  def apply(baseUrl: String = "https://api.hetzner.cloud/v1"): Actions[hcloud.Authorization.NoAuthorization.type] = Actions(baseUrl, hcloud.Authorization.NoAuthorization)
  def withBasicAuth(baseUrl: String, username: String, password: String): Actions[hcloud.Authorization.BasicAuth] =
    Actions(baseUrl, hcloud.Authorization.BasicAuth(username, password))

  def withApiKeyAuth(baseUrl: String, apiKey: String): Actions[hcloud.Authorization.ApiKey] =
    Actions(baseUrl, hcloud.Authorization.ApiKey(apiKey))

  def withBearerTokenAuth(baseUrl: String, token: String): Actions[hcloud.Authorization.BearerToken] =
    Actions(baseUrl, hcloud.Authorization.BearerToken(token))

case class Actions[Auth <: hcloud.Authorization] private (baseUrl: String, authConfig: hcloud.Authorization):
  def withBasicAuth(username: String, password: String): Actions[hcloud.Authorization.BasicAuth] =
  ...
  def withBearerTokenAuth(token: String): Actions[hcloud.Authorization.BearerToken] =
   ...
   */
  def getAction(id: Long)(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], GetActionResponse]] =
   ...
   * 
   * Expected answers:
   *   code 200 : GetMultipleActionsResponse (Response for listing Actions.)
   * 
   * Available security schemes:
   *   APIToken (http)
   * 
   ...
   */
  def getMultipleActions(id: Seq[Long])(using Auth <:< hcloud.Authorization.BearerToken): sttp.client4.Request[Either[ResponseException[String], GetMultipleActionsResponse]] =
    val requestURL =
      uri"$baseUrl/actions"
