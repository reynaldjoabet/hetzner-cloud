...
import hcloud.models.DeleteRouteRequest
import hcloud.models.DeleteSubnetRequest
...
object NetworkActions:
  ...
  ): NetworkActions[hcloud.Authorization.NoAuthorization.type] =
    NetworkActions(baseUrl, hcloud.Authorization.NoAuthorization)
  def withBasicAuth(
      baseUrl: String,
      username: String,
      password: String
  ): NetworkActions[hcloud.Authorization.BasicAuth] =
    NetworkActions(baseUrl, hcloud.Authorization.BasicAuth(username, password))

  def withApiKeyAuth(
      baseUrl: String,
      apiKey: String
  ): NetworkActions[hcloud.Authorization.ApiKey] =
    NetworkActions(baseUrl, hcloud.Authorization.ApiKey(apiKey))

  def withBearerTokenAuth(
      baseUrl: String,
      token: String
  ): NetworkActions[hcloud.Authorization.BearerToken] =
    NetworkActions(baseUrl, hcloud.Authorization.BearerToken(token))

case class NetworkActions[Auth <: hcloud.Authorization] private (
    baseUrl: String,
  ...
  def withNoAuth: NetworkActions[hcloud.Authorization.NoAuthorization.type] =
  ...
  /** Adds a route entry to a [Network](#tag/networks). If a change is currently
    * being performed on this [Network](#tag/networks), a error response with
    ...
    */
  def addNetworkRoute(id: Long, addRouteRequest: AddRouteRequest)(using
  ...
  /** Adds a new subnet to the [Network](#tag/networks). If the subnet
    * `ip_range` is not provided, the first available `/24` IP range will be
    * used. If a change is currently being performed on this
    * [Network](#tag/networks), a error response with code `conflict` will be
    * returned.
    *
    * Expected answers: code 201 : ActionResponse1 (Request succeeded.) code 4xx
    * : GetActions4xxResponse (Request failed with a user error.) code 5xx :
    ...
    */
  def addNetworkSubnet(id: Long, addSubnetRequest: AddSubnetRequest)(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[Either[ResponseException[String], ActionResponse1]] =
    val idPathParam =
      PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
  ...
  /** Changes the IP range of a [Network](#tag/networks). The following
    * restrictions apply to changing the IP range: - IP ranges can only be
    * extended and never shrunk. - IPs can only be added to the end of the
    * existing range, therefore only the netmask is allowed to be changed. To
    * update the routes on the connected [Servers](#tag/servers), they need to
    * be rebooted or the routes to be updated manually. For example if the
    * [Network](#tag/networks) has a range of `10.0.0.0/16` to extend it the new
    * range has to start with the IP `10.0.0.0` as well. The netmask `/16` can
    * be changed to a smaller one then `16` therefore increasing the IP range. A
    * valid entry would be `10.0.0.0/15`, `10.0.0.0/14` or `10.0.0.0/13` and so
    * on. If a change is currently being performed on this
    * [Network](#tag/networks), a error response with code `conflict` will be
    * returned.
    *
    * Expected answers: code 201 : ActionResponse1 (Request succeeded.) code 4xx
  ...
  /** Changes the protection settings of a [Network](#tag/networks). If a change
    * is currently being performed on this [Network](#tag/networks), a error
  ...
  /** Delete a route entry from a [Network](#tag/networks). If a change is
    * currently being performed on this [Network](#tag/networks), a error
    * response with code `conflict` will be returned.
    *
    * Expected answers: code 201 : ActionResponse1 (Request succeeded.) code 4xx
    * : GetActions4xxResponse (Request failed with a user error.) code 5xx :
    ...
    */
  def deleteNetworkRoute(id: Long, deleteRouteRequest: DeleteRouteRequest)(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[Either[ResponseException[String], ActionResponse1]] =
    val idPathParam =
      PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
  ...
  /** Deletes a single subnet entry from a [Network](#tag/networks). Subnets
    * containing attached resources can not be deleted, they must be detached
    * beforehand. If a change is currently being performed on this
    * [Network](#tag/networks), a error response with code `conflict` will be
    * returned.
    *
    * Expected answers: code 201 : ActionResponse1 (Request succeeded.) code 4xx
    ...
    */
  def deleteNetworkSubnet(id: Long, deleteSubnetRequest: DeleteSubnetRequest)(
  ...
  /** Returns a specific [Action](#tag/actions) for a [Network](#tag/networks).
    ...
    *   ID of the Action.
    */
  def getNetworkAction(id: Long, actionId: Long)(using
      Auth <:< hcloud.Authorization.BearerToken
  ...
  /** Returns a single [Action](#tag/actions).
    ...
    *   ID of the Action.
    */
  def getNetworksAction(id: Long)(using
      Auth <:< hcloud.Authorization.BearerToken
  ): sttp.client4.Request[Either[ResponseException[String], ActionResponse]] =
    val idPathParam =
      PathSerializable.serialize("id", id, PathStyleFormat.SIMPLE, false)
  ...
  /** Lists [Actions](#tag/actions) for a [Network](#tag/networks). Use the
    * provided URI parameters to modify the result.
    *
  ...
  /** Lists multiple [Actions](#tag/actions). Use the provided URI parameters to
    * modify the result.
    *
    * Expected answers: code 200 : ActionListResponseWithMeta (Request
    * succeeded.) code 4xx : GetActions4xxResponse (Request failed with a user
