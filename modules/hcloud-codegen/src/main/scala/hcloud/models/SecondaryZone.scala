...
case class SecondaryZone(
    ...
    @named("status") status: SecondaryZoneEnums.Status,
    /* Number of resource records (RR) within the [Zone](#tag/zones). */
    @named("record_count") recordCount: Int,
    @named(
      "authoritative_nameservers"
    ) authoritativeNameservers: PrimaryZoneAllOfAuthoritativeNameservers,
    /* Registrar of the domain. */
    @named("registrar") registrar: SecondaryZoneEnums.Registrar,
    /* Mode of the [Zone](#tag/zones).  For more information, see [Zone Modes](#tag/zones/zone-modes).  */
    @named("mode") mode: SecondaryZoneEnums.Mode,
    ...
    @named("primary_nameservers") primaryNameservers: Option[
      Seq[PrimaryZoneAllOfPrimaryNameserversInner]
    ] = scala.None
)

object SecondaryZoneEnums:
  enum Status:
    case `ok`
    case `updating`
    case `error`
