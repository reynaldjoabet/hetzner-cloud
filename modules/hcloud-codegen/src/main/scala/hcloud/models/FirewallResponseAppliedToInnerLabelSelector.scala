...
/** [Label Selector](#description/label-selector) the [Firewall](#tag/firewalls)
  * is applied to. Only set for `type` `label_selector`, otherwise `null`.
  */
case class FirewallResponseAppliedToInnerLabelSelector(
    /* The selector. */
    @named("selector") selector: String
)
