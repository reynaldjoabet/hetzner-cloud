...
case class FirewallResponseAppliedToInner(
    /* The type of resource to apply. */
    @named("type") `type`: FirewallResponseAppliedToInnerEnums.Type,
    @named("server") server: Option[FirewallResponseAppliedToInnerServer] =
      scala.None,
    @named("label_selector") labelSelector: Option[
      FirewallResponseAppliedToInnerLabelSelector
    ] = scala.None,
    /* Resources applied to via this [Label Selector](#description/label-selector).  */
    @named("applied_to_resources") appliedToResources: Option[
      Seq[FirewallResponseAppliedToInnerAppliedToResourcesInner]
    ] = scala.None
)

object FirewallResponseAppliedToInnerEnums:
  enum Type:
    case `server`
    case `label_selector`
...
end FirewallResponseAppliedToInnerEnums
