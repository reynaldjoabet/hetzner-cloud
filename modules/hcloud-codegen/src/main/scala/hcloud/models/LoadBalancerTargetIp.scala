...
/** LoadBalancerTargetIP IP target where the traffic should be routed to. It is
  * only possible to use the (Public or vSwitch) IPs of Hetzner Online Root
  ...
  * (Servers, Load Balancers, ...) are blocked as well. Only present for target
  * type `ip`.
  */
case class LoadBalancerTargetIP(
