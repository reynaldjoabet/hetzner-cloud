...
/** Primary nameserver that returns [Zones](#tag/zones) via `AXFR`. Must allow
  * queries from and may send `NOTIFY` queries to [Hetzner's secondary
  ...
  */
case class PrimaryZoneAllOfPrimaryNameserversInner(
    ...
    @named("tsig_algorithm") tsigAlgorithm: Option[
      PrimaryZoneAllOfPrimaryNameserversInnerEnums.TsigAlgorithm
    ] = scala.None
)

object PrimaryZoneAllOfPrimaryNameserversInnerEnums:
  enum TsigAlgorithm:
    case `hmac-md5`
    case `hmac-sha1`
    case `hmac-sha256`
...
end PrimaryZoneAllOfPrimaryNameserversInnerEnums
