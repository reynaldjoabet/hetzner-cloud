...
/** LoadBalancerTargetIP Configuration for an IP target. It is only possible to
  * use the (Public or vSwitch) IPs of Hetzner Online Root Servers belonging to
  * the project owner. IPs belonging to other users are blocked. Additionally
  * IPs belonging to services provided by Hetzner Cloud (Servers, Load
  * Balancers, ...) are blocked as well. Only valid and required if type is
  * `ip`.
  */
case class LoadBalancerTargetIP1(
