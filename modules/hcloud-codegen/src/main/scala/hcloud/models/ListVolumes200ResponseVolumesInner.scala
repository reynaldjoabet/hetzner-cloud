...
case class ListVolumes200ResponseVolumesInner(
    ...
    @named("location") location: ListVolumes200ResponseVolumesInnerLocation,
    ...
    @named("status") status: ListVolumes200ResponseVolumesInnerEnums.Status,
    /* Filesystem of the Volume if formatted on creation, null if not formatted on creation. */
    @named("format") format: String
)

object ListVolumes200ResponseVolumesInnerEnums:
  enum Status:
    case `available`
    case `creating`
...
end ListVolumes200ResponseVolumesInnerEnums
