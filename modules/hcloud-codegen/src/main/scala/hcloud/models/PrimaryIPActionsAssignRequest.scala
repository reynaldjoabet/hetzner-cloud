...
case class PrimaryIPActionsAssignRequest(
    /* Type of resource assigning the Primary IP to. */
    @named(
      "assignee_type"
    ) assigneeType: PrimaryIPActionsAssignRequestEnums.AssigneeType,
    /* ID of a resource of type `assignee_type`. */
    @named("assignee_id") assigneeId: Long
)

object PrimaryIPActionsAssignRequestEnums:
  enum AssigneeType:
    case `server`
...
end PrimaryIPActionsAssignRequestEnums
