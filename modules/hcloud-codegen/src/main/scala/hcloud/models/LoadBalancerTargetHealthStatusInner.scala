...
case class LoadBalancerTargetHealthStatusInner(
    @named("listen_port") listenPort: Option[Int] = scala.None,
    @named("status") status: Option[
      LoadBalancerTargetHealthStatusInnerEnums.Status
    ] = scala.None
)

object LoadBalancerTargetHealthStatusInnerEnums:
  enum Status:
    case `healthy`
    case `unhealthy`
    case `unknown`
...
end LoadBalancerTargetHealthStatusInnerEnums
