...
case class ActionNullable(
    ...
    @named("finished") finished: OffsetDateTime,
    /* Progress of the Action in percent. */
    @named("progress") progress: Int,
    /* Resources the Action relates to. */
    @named("resources") resources: Seq[ActionResourcesInner],
    @named("error") error: ActionError
)

object ActionNullableEnums:
  enum Status:
    case `running`
    case `success`
    case `error`
